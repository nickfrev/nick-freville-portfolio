	...
	draw() {
		for (let i = 0; i < this.maxObjs; i++) {
			if (this.objects[i].exists && this.objects[i].visible) {
				this.shader(this.objects[i]);
			}
		}
	}

	shader(inObj) {
		const inMesh = inObj.mesh;
		const inPos = inObj.pos;
		//print the verts
		inMesh.isCulled = true;
		this.ctx.fillStyle = inObj.color;
		for (let p = 0; p < inMesh.verts.length; p++) {
			const curPos = {
			...
			if (dotAng(tmp, this.cam.up) > 1.5708) {
				...
				if (!isNaN(curProj.x) && !isNaN(curProj.x)) {
					...
					if (
						...
						this.ctx.fillRect(
							inMesh.verts[p].drawx,
							inMesh.verts[p].drawy,
							(-1500 / curProj.d.z) * inObj.weight,
							(-1500 / curProj.d.z) * inObj.weight,
						);
					} else {
